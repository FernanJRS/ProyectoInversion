...
namespace ProyectoInversion.Modulos.GestiónEscenarios
{
    public partial class frmCompararEscenarios : Form
    {
        ...
        String metrica = "VAN";
        private int _simulacionID;
        private int _alternativaBaseID;
        public frmCompararEscenarios(int simulacionID)
        {
            ...
            try
            {
                ...
                if (dvFiltrado.Table.Rows.Count > 0)
                {
                    ...
                    if (cmbAlternativaA.Items.Count > 0) cmbAlternativaA.SelectedIndex = 0;
                    ...
                    if (cmbAlternativaB.Items.Count > 1) cmbAlternativaB.SelectedIndex = 1; 

                    // Una vez cargados, disparar la primera comparativa
                    //this.Shown += frmCompararEscenarios_Shown;
                }
                _simulacionID = simulacionID;
                _alternativaBaseID = Convert.ToInt32(cmbEscenarioBase.SelectedValue);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error al cargar alternativas: {ex.Message}");
            }
        ...
        private void CargarDatosComparativa()
        {
            ...
            {
                MessageBox.Show($"Error al cargar Alternativa Base: {ex.Message}");
                return;
            }
            ...
            try
            {
                DataTable dtA = db.ObtenerDetalleAlternativa(_simulacionID,idA);
            ...
            {
                MessageBox.Show($"Error al cargar Alternativa A: {ex.Message}");
                return;
            }

            try
            {
                DataTable dtB = db.ObtenerDetalleAlternativa(_simulacionID, idB);
            ...
            {
                MessageBox.Show($"Error al cargar Alternativa B: {ex.Message}");
                return;
            }
        ...
        private void ActualizarGraficoComparativo(double vBase, double vA, double vB, String metrica, String formato)
        {
            ...
            {
                MessageBox.Show($"Error al graficar comparativa: {ex.Message}");
            }
        }
        ...
        private void AbrirCrearAlternativa(int alternativaID, string panelOrigen)
        {
            ...
            using (frmCrearAlternativa fCrear = new frmCrearAlternativa(_simulacionID, _alternativaBaseID, alternativaID, panelOrigen))
            {
                fCrear.Owner = this;
